...
def ll [--all(-a), ...pattern]  {
  let pattern = if ($pattern | is-empty) { '.' } else { $pattern | str join " " }
  if $all  {
    ls -la $pattern | select name mode user group size
  } else {
    ls -l $pattern | select name mode user group size
  }
}
